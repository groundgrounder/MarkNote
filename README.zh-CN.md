...
- LaTeX 公式：`$$…$$` 与单个 `$…$` 都渲染为行内公式，独占一行的 `$$` 渲染为居中的块级公式——矩阵（`\begin{pmatrix} a & b \\ c & d \end{pmatrix}`）、分式、积分、求和、希腊字母等常见 TeX 命令都可用。公式取主题的文字颜色，深色主题下不会变成黑字黑底；字号跟随预览字号。分隔符按 Pandoc 的规则判定，「从 $5 到 $10」「$ x$」「$x $」「$x$1」都保持原样。渲染不出来的公式会以错误色 + 描边框显示公式源，而不是无声地留成普通文字
...
- 渲染不出来的公式会以错误色 + 描边框显示公式源，而不是无声地留成普通文字。一个未定义的命令（`\zzzz`）仍会让整个公式失败（库就是这么解析的），但至少能看出是哪一条写坏了
