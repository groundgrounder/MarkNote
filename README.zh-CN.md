...
- 撤销/重做：顶栏两个按钮（连续输入按停顿合并成一步：600ms 内同类改动并成一条；工具栏插入与替换全部各自单独成步；撤销后光标回到改动处，撤到底按钮自动置灰）。不做 Ctrl+Z 快捷键：文本框有焦点时字母按键先交给输入法，Gboard 自带逐字符撤销会吃掉这个组合，应用侧收不到——Activity、pre-IME、Compose 三层钩子都实测过
...
- LaTeX 公式：`$$…$$` 渲染为行内公式，独占一行的 `$$` 渲染为居中的块级公式——矩阵（`\begin{pmatrix} a & b \\ c & d \end{pmatrix}`）、分式、积分、求和、希腊字母等常见 TeX 命令都可用。公式取主题的文字颜色，深色主题下不会变成黑字黑底；字号跟随预览字号。只有 `$$` 算公式，单个 `$` 原样保留，所以「从 $5 到 $10」这类文字不会被误判
- 只读预览同样能搜索、也能调大纲：搜索跑在**渲染结果**上（所见即所搜），命中逐个高亮、当前命中实心高亮并自动滚入视野、n/m 计数与上下跳转；大纲点击直接把正文滚到对应标题。预览是只读视图，所以搜索面板不提供替换
- 图片显示：相对路径图片按**文档所在文件夹**解析（`..` 可正常上行，与 Markdown 惯例一致），首次按引导条授权一次即可——请选**同时包含文档与图片**的文件夹（授权长期有效）；图片落在授权范围外或找不到时，会在原位置显示「已授权文件夹里没有这张图」的占位，而不是静默消失；支持 `content://`、`file://`、base64 内嵌图；大图自动降采样防 OOM
...
└── ui/
    ...
    ├── settings/                # 设置页（含语言切换）
    └── editor/                  # 编辑器页、工具栏、语法高亮、大纲、撤销栈、Markwon 预览
```

另有 `app/src/main/res/xml/locales_config.xml` —— Android 13+ 系统「应用语言」的可选清单，由清单里的 `android:localeConfig` 引用。

改了编辑器的文本改动逻辑后，跑一次 `tools/run_checks.sh`：它把 `UndoStack`（纯 Kotlin、零 Android 依赖）编出来，用 JVM 断言把差分边界、合并规则、栈上限与自救清栈跑一遍，不需要模拟器，秒级出结果。
...
- 编辑器支持撤销/重做，入口是顶栏两个按钮（只在编辑态出现，预览态没有可撤销的操作）。历史用自研的差分栈实现：纯 Kotlin、零 Android 依赖，由 `tools/run_checks.sh` 里的 JVM 断言覆盖
- 连续输入会合并成一步（600ms 内的同类改动并成一条，停顿即断），工具栏插入与替换全部则各自单独成步。撤销后光标回到改动处，撤到底按钮自动置灰
- 栈有双重上限（200 条 + 差分总字符 500k），一次大段粘贴也不会把它撑到无界。每次撤销/重做前都会核对差分与当前正文是否还对得上——过期的那条会清空历史，而不是静默改坏文档
- 整份替换正文会让历史作废：打开/重新载入文档，以及没有未保存改动时重新读盘，这两条路都会清栈，所以「重新读盘」不可能被一段过期的差分撤销掉
- 不做 Ctrl+Z 快捷键，这是有意为之：文本框有焦点时字母按键先交给输入法，Gboard 自带逐字符撤销会吃掉这个组合，应用侧收不到。Activity、pre-IME、Compose 三层钩子都实测过，于是把 handler 删掉，而不是留作死代码
- 另外两处清理：设置页的版本号改为走多语言字符串资源，编辑器页一个没用到的图标 import 已删
...
- 修复：相对路径图片会无声消失。路径是按**授权文件夹的根**拼的、不是相对文档所在目录，而且 `..` 被直接丢弃而不是上行——于是 `![](../img/a.png)`、以及文档不在授权文件夹根部的那一大类写法，都会解析到错位置；更糟的是那个位置恰好有同名文件时，它会**加载错误的图且毫无提示**。现在按文档所在目录解析（`..` 正常上行，与 Markdown 惯例一致），找不到再退回授权根，已经能用的写法不受影响
- 加载失败不再无声：以前图片位置只剩一个破图小方块——Markwon 只回落到 alt 文本，而 `![]()` 的 alt 是空的，所以什么都没有。现在会把失败原因画在图片该在的位置上
- 授权失效时会重新露出引导条：重装或权限被撤销后，文件夹 URI 可能还记在列表里、授权本身已经没了，此前既不提示也不显示，纯静默
- 引导条文案改为说明该授哪个文件夹：**同时包含文档与图片**的那个。旧文案指向「文档所在文件夹」，图片在它之外时照做必然失败

### v1.1.4

- 撤销不再吞掉工具栏插入之后的输入：工具栏插入、替换全部这类「独立操作」现在是**双向屏障**，按完 H1 再打字不会并成一步（否则撤销一次会把两件事一起退掉）。原来的断言只覆盖了这条规则的一个方向
- 编辑器重新进入文档时，可能把**刚敲进去的字**整份覆盖掉：重读的守卫是在（挂起的）读盘**之前**判定的，读盘期间输入的内容会被磁盘内容替换
- 大纲现在同时认 `~~~` 与 ``` ``` ``` 两种围栏，且只有**同种字符、长度不短于开头**才算闭合——此前 `~~~` 块里的 `#` 会漏进大纲，而块内的 ``` ``` ``` 又会把代码块提前闭合
- 搜索命中数与「全部替换」的处数不再对不上：计数以前会把**重叠**命中也算进去（`aaaa` 搜 `aa` 报 3 处，实际只换 2 处）
- 带 BOM 的文件，列表摘要不再以一个零宽 U+FEFF 开头
- 最近打开列表上限 100 条（超出按打开时间淘汰最旧的）——渲染列表时每条都要两次跨进程查询，此前越用越慢
- provider 不支持截断写时，保存会核对文件长度，避免写短了却把旧内容的尾巴留在文件里
- 最近列表的 JSON 解析失败时先把原文备份下来，而不是静默把列表清空
...
- 公式是异步渲染的——先短暂显示公式源码、再替换为排版结果，所以公式多的文档在渲染期间不会卡住预览
- `$$` 分隔符不属于渲染文本，因此在预览里搜 `$$` 搜不到、在编辑器里能搜到；公式内部的内容照常可搜
- 修复：大纲跳到「标题本身含公式」的小节时落点不准。源码偏移换算成渲染偏移的那一步靠「在渲染文本里找标题文字」，而它不知道渲染时 `$$` 会被剥掉——于是查找落空、退化成按比例估算的大概位置
